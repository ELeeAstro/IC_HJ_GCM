...
module k_Rosseland_mod
  ...
  !private
  public :: k_Ross_Freedman, k_Ross_Valencia, gam_Parmentier, Bond_Parmentier, k_Ross_Tan
...
contains
  ...
  !! This is a modified version from the internally run GCM one - modifed to find IC profile
  ...
  subroutine gam_Parmentier(Teff, table_num, gam_V, Beta_V, Beta, gam_1, gam_2, gam_P, tau_lim)
    ...
    ! gamma_Planck - if < 1 then make it (very close to) grey approximation (k_Planck = k_Ross, gam_P = 1)
    gam_P = 10.0_dp**(aP * l10T2 + bP * l10T + cP)
    if (gam_P < (1.0_dp + 1.0e-6_dp)) then
      gam_P = 1.0_dp + 1.0e-6_dp
    end if
